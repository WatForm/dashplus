...
public class TransPreD2A extends InvsD2A {
    ...
    */
    ...
    public void addTransPre(String tfqn) {
        ...
        if (this.dm.whenR(tfqn) != null)
            body.add(this.translateExpr(this.dm.whenR(tfqn)));
        ...
        if (this.dm.hasConcurrency() && 
        	this.dm.onR(tfqn) != null && 
        	this.dm.hasEnvEvents()) {
            // trig_events_t1
            ...
            if (this.dm.isIntEvent(ev.name)) {
                ...
                ifBranch = AlloyIn(ev.asAlloyArrow(),
                        AlloyRangeRes(
        ...
        for (String t:priTrans) {
            ...
            args = 
            	this.dsl.curParamVars(this.dm.transParams(t));
            body.add(
            	AlloyNot(
            		AlloyPredCall(DashFQN.translateFQN(t)+D2AStrings.preName, args)));
        }
        
        this.addPred(
        	tout+D2AStrings.preName, 
        	this.dsl.curParamsDecls(params), body); 
