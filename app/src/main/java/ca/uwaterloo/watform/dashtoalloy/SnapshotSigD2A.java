    ...
    for (int i = 0; i <= this.dm.maxDepthParams(); i++) {
      ...
      if (this.dm.hasTransAti(i)) {
        if (i == 0) {
          decls.add(
              new AlloyDecl(
                  this.dsl.transTakenVar(0), AlloyQtEnum.LONE, this.dsl.transLabelNameVar()));
        } else {
          ...
          for (int x = 1; x < i; x++)
            // PID set -> set arrow
            arrow =
                new AlloyArrowExpr(AlloyVar(cop.get(x)), AlloyQtEnum.SET, AlloyQtEnum.SET, arrow);
      ...
      if (this.dm.hasEvents() && this.dm.hasEventsAti(i)) {
        ...
        AlloyExpr allPossibleEventsAti =
            AlloyUnion(mapBy(eventsAtThisLevel, ev -> this.translateDashRefToArrowExpr(ev)));
        // fact: all s. s.eventsi in PID1 -> ev1 + PID2 -> ev2
        eventLimitations.add(
            AlloyAllVars(
                this.dsl.curDecls(), AlloyIn(this.dsl.curEvents(i), allPossibleEventsAti)));
      }
    }
    ...
    for (String vfqn : this.dm.allVarNames()) {
      ...
      if (this.dm.params(vfqn).isEmpty()) {
        ...
        for (int i = 1; i < PIDList.size(); i++) {
          ...
          arrow = new AlloyArrowExpr(arrow, AlloyQtEnum.SET, AlloyQtEnum.SET, PIDList.get(i));
        }
