...
public final class DashStateItemParseVis extends DashBaseVisitor<DashStateItem> {
  ...
  public DashBufferDecls visitDashBufferDecls(DashParser.DashBufferDeclsContext ctx) {
    return new DashBufferDecls(
        new Pos(ctx),
        this.extractNames(ctx.names()),
        (null != ctx.qname()) ? this.extractName(ctx.qname()) : new AlloySigIntExpr().toString(),
  ...
  public DashTrans visitDashTrans(DashParser.DashTransContext ctx) {
    ...
    return new DashTrans(
        ...
        (DashFrom) extractOneFromList(extractItemsOfClass(transItems, DashFrom.class), "DashFrom"),
        (DashGoto) extractOneFromList(extractItemsOfClass(transItems, DashGoto.class), "DashGoto"),
        (DashOn) extractOneFromList(extractItemsOfClass(transItems, DashOn.class), "DashOn"),
        (DashSend) extractOneFromList(extractItemsOfClass(transItems, DashSend.class), "DashSend"),
        (DashWhen) extractOneFromList(extractItemsOfClass(transItems, DashWhen.class), "DashWhen"),
        (DashDo) extractOneFromList(extractItemsOfClass(transItems, DashDo.class), "DashDo"));
  }
