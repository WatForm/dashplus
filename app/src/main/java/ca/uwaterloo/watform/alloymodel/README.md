...
* This package contains a *sequence* of classes that start with classes that are a **semantic** model of everything in the AlloyModel and its imports; and are followed by a **syntactic** model of elements within "this" namespace (top-level file). The advantage of making them a sequence is ease of making accessors for other packages - rather than am.allSigs() redirect to am.sigs.allSigs().
...
* API calls can only add to THIS_NAMESPACE
...
* DONE SMBase
    - contains useful constants and functions for namespaces
...
* DONE SMFields 
    ...
    - (*init*) already empty; does nothing
    - (*createField(name, namespace(includes parentSig, boundingExpr)*) set unresolved flag
    - (*lookup for resolve*) 
...
* DONE SMPredFuns 
    ...
    - (*init*) TODO contains built-ins (e.g., pred/totalOrder, fun/sum) with no body
    - (*createPred(name, namespace, args, body)*)
    - (*createFun(name, namespace, args, returnType, body)*)
    - (*lookup for resolve*) may return multiple entries for name
    - (*resolve*) 
        - resolve1 on arg types, return types
        - resolve2 on body - must consider namespace 
...
* DONE: SMConstraints 
    ...
    - have a namespace associated with each fact
    - (*init*) already empty; does nothing
    - (*createConstraint(name, namespace, expr)*)
    - (*resolve2*) walk over all (field decl facts, sig facts, all other facts)
    - **TODO**: do we need a kind (sigfact, bounding expression, fact) ?
...
* use 'add' methods for public API additions

* DONE AMThisEnumParas init/API
    - adds to SMSigs
    - set ordering and scope in SemCmds
    - no fields or facts allowed in enums

* DONE AMThisSigParas init/API
    ...
    - desugared sig facts
        `this in g` becomes all this:B | this in this.g
        added to SMConstraints with src

* DONE AMThisPredParas init/API
    - add to SMPredFuns

* DONE AMThisFunParas init/API
    - add to SMPredFuns

* DONE: AMThisFactPara init/API
    - adds to SMConstraints

* DONE AMThisAssertPara init/API
    - adds to SMCmds in namespace

* DONE AMThisCmdPara init/API
    - add cmdDelc to SMCmds in namespaces

* DONE AMThisImportPara init/API
...
* DONE AMThisModulePara init/API
    - nothing to be done
...
* SigData, FieldData, CmdData, PredFunData -- classes to store info about each Base element
