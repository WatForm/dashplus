  ...
  class ResolveVis implements AlloyExprVis<ResolveInfo> {
    ...
    */
    ...
    private void equalArities(ResolveInfo leftResult, ResolveInfo rightResult) {
      if (!leftResult.arity.equals(rightResult.arity)) {
        ...
        throw AlloyModelError.arityMismatch(
            leftResult.exp.pos,
            leftResult.exp.toString() + "(" + Integer.toString(leftResult.arity.get()) + ")",
            rightResult.exp.toString() + "(" + Integer.toString(rightResult.arity.get()) + ")");
      }
    }
    ...
    public ResolveInfo visit(AlloyArrowExpr binExpr) {
      ...
      AlloyExpr resultExp =
          new AlloyArrowExpr(
              binExpr.pos, leftResult.exp, mul1.orElse(null), mul2.orElse(null), rightResult.exp);
    ...
    public ResolveInfo visit(AlloyDotExpr binExpr) {
      ...
      if (!leftResult.argArities.isEmpty())
        throw AlloyModelError.missingArgsToPredFunCall(binExpr.left.pos, binExpr.left.toString());
    ...
    public ResolveInfo visit(AlloyShAExpr binExpr) {
      throw ImplementationError.notSupported("AlloyShA inside ArityVis yet: " + binExpr.toString());
    }

    @Override
    public ResolveInfo visit(AlloyShLExpr binExpr) {
      throw ImplementationError.notSupported("AlloyShL inside ArityVis yet: " + binExpr.toString());
    }

    @Override
    public ResolveInfo visit(AlloyShRExpr binExpr) {
      throw ImplementationError.notSupported("AlloyShR inside ArityVis yet: " + binExpr.toString());
    }
    ...
    public ResolveInfo visit(AlloyBracketExpr bracketExpr) {
      ...
      AlloyExpr resultExpr =
          new AlloyBracketExpr(bracketExpr.pos, exprResult.exp, mapBy(exprsResult, r -> r.exp));
    ...
    public ResolveInfo visit(AlloyCphExpr comprehensionExpr) {
      ...
      if (comprehensionExpr.body.isPresent()) {
        ...
        throw AlloyModelError.mustBeFormula(
              ...
              comprehensionExpr.body.toString() + " of arity " + bodyResult.arity.toString());
    ...
    public ResolveInfo visit(AlloyIteExpr iteExpr) {
      ...
      if (!condResult.arity.equals(ONE_ARITY)) { // !condResult.arity.equals(UNKNOWN_ARITY) && ) {
    ...
    public ResolveInfo visit(AlloyQuantificationExpr quantificationExpr) {
      ...
      return new ResolveInfo(
          ...
          new AlloyQuantificationExpr(
              quantificationExpr.pos, quantificationExpr.quant, newDecls, bodyResult.exp));
    }
        ...
        if (SMResolve.this.isSig(qname)) {
          ...
          if (SMResolve.this.sigParentOfField.isPresent()) {
            ...
            if (!SMResolve.this.isFieldResolved(chosen)) {
              ...
              throw AlloyModelError.cannotRefFieldInBoundingExprOutsideOfItsSig(
                  varExpr.getName(), SMResolve.this.sigParentOfField.get().fullName());
            } else {
          ...
          */
          ...
          if (returnArity.isPresent()) {
            List<Optional<Integer>> argsArities = SMResolve.this.predFunArgArities(chosen);
            return new ResolveInfo(
                argsArities, returnArity, chosen.toAlloyExpr(varExpr.pos, Kind.PREDFUN));
          } else {
            throw AlloyModelError.unknownName(varExpr.pos, varExpr.toString());
          }
        } else {
