    ...
    for (Qname sigQname : this.allSigQnames()) {
      ...
      if (!this.inParents(sigQname).isEmpty()) {
        ...
        if (!this.extendsChildren(sigQname).isEmpty()) {
          ...
          throw AlloyModelError.cantExtendSubsetSig(this.pos(childQname), childQname.toString());
        }
      }
    }
  }
  ...
  private void setChildren() {
    ...
    for (Qname sigQname : this.allSigQnames()) {
      ...
      if (this.sigTable.get(sigQname).extendsParent.isPresent()) {
        this.sigTable.get(sigTable.get(sigQname).extendsParent.get()).extendsChildren.add(sigQname);
      }
    }
  }
