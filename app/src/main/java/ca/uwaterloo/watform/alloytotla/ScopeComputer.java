...
public class ScopeComputer {
    ...
    */
    ...
    public ScopeComputer(AlloyModel alloyModel, int max) {
        ...
        for (String s : alloyModel.allSigs()) {
            if (alloyModel.isOneSig(s)) equalConstraint(s, 1);
        }
    }
    ...
    public void maxConstraint(String s, int n) {
        ...
        if (maxima.get(s) < n) maxima.put(s, n);

        // if sig t in sig s, then max(t) <= max(s)
        for (String t : alloyModel.inChildren(s)) {
            maxConstraint(t, n);
        }

        // if sig t1, t2... extends sig s, max(ti) <= max(s) - sum of min(tj) where j!=i
        for (String t : alloyModel.extendsChildren(s)) {
    ...
    public void minConstraint(String s, int n) {
        ...
        if (minima.get(s) > n) minima.put(s, n);

        // if sig s in sig t, then min(t) >= min(s)
        for (String t : alloyModel.inParents(s)) {
            minConstraint(t, n);
        }
        ...
        alloyModel
                ...
                .ifPresent(
                        t -> {
                            int sum = 0;
                            for (String u : alloyModel.extendsChildren(t)) sum += minima.get(u);
                            minConstraint(t, sum);
                        });
    }
