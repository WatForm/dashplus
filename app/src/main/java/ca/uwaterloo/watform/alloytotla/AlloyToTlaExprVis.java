...
public class AlloyToTlaExprVis implements AlloyExprVis<AlloyToTlaExprVis.Result> {
      ...
      case MacroResult macroResultRight:
        {
          ...
          if (args.size() == macroResultRight.numArgs) {
            ...
            return new TlaExpResult(answer);
          } else return new MacroResult(macroResultRight.numArgs, macroResultRight.name, args);
        }
    }
  }
  ...
  public Result visit(AlloyBinaryExpr binExpr) {
    ...
    throw ImplementationError.notSupported("non-translatable expression: " + binExpr.toString());
  }
  ...
  public Result visit(AlloyUnaryExpr unaryExpr) {
    ...
    throw ImplementationError.notSupported("non-translatable expression: " + unaryExpr.toString());
  }
  ...
  public Result visit(AlloyVarExpr varExpr) {
    ...
    throw ImplementationError.notSupported("non-translatable expression: " + varExpr.toString());
  }
  ...
  public Result visit(AlloyCphExpr comprehensionExpr) {
    ...
    */
    ...
    var head =
        vars.size() == 1 ? TlaQuantOpHeadFlat(vars, product) : TlaQuantOpHeadTuple(vars, product);
  ...
  public Result visit(AlloyLetExpr letExpr) {
    ...
    */
    ...
    var asns =
        mapBy(letExpr.asns, asn -> TlaDefn(TlaDecl(asn.qname.label), extract(visit(asn.expr))));
    ...
    */
    ...
    switch (quantificationExpr.quant) {
      case AlloyQuantificationExpr.Quant.ALL:
        {
          for (var decl : quantificationExpr.decls) {
            var head = TlaQuantOpHead(TlaVar(decl.getName()), extract(visit(decl.expr)));
      ...
      case AlloyQuantificationExpr.Quant.SUM:
        {
          throw ImplementationError.notSupported("Unsupported translation for sum quantification");
        }
      default:
        {
          List<TlaVar> headVars = mapBy(quantificationExpr.decls, decl -> TlaVar(decl.getName()));
