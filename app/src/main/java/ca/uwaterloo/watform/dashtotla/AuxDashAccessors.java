...
public class AuxDashAccessors {
    ...
    public static List<String> getLeafStateNames(DashModel dm) {
        return new ArrayList<>(); //GeneralUtil.filterBy(getAllStateNames(dm), x -> dm.st.isLeaf(x));
    }
    ...
    public static List<String> getDescendantStateNames(String stateName, DashModel dm) {
        return new ArrayList<>(); //GeneralUtil.filterBy(dm.st.getAllNames(), name -> name.startsWith(stateName));
    }
    ...
    public static List<String> initialEntered(DashModel dashModel) {
        return new ArrayList<>(); //mapBy(dashModel.st.getRootLeafStatesEntered(), x -> x.toString());
    }
}
