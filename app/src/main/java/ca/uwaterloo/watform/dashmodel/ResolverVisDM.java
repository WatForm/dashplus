...
public class ResolverVisDM extends InitializeDM implements AlloyExprVis<AlloyExpr> {
  ...
  private AlloyExpr resolve(AlloyVarExpr varExpr, List<? extends AlloyExpr> v_param_vals) {
    ...
    */
    ...
    if (matches.isEmpty()) {
      if (!v_param_vals.isEmpty()) throw DashModelError.unknownElementWithParamsError(varExpr);
  ...
  public AlloyExpr visit(AlloyUnaryExpr unaryExpr) {
    if (unaryExpr.op == AlloyStrings.PRIME) {
      ...
      // value can be primed)
      if (!(newExpr instanceof DashRef)) throw DashModelError.cantNextNonDynamicVarError(unaryExpr);
      else {
        // is DashRef
  ...
  public AlloyExpr visit(DashRef dashRef) {
    ...
    AlloyExpr newExpr = resolve(new AlloyQnameExpr(dashRef.pos, dashRef.name), resolvedParamValues);
