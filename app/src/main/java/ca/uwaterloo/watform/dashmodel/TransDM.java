...
public class TransDM extends InitsInvsDM {
  ...
  public List<String> eventsThatTriggerTrans() {
    return mapBy(
        filterBy(allTransNames(), i -> this.tt.get(i).onR != null), i -> this.tt.get(i).onR.name);
  }
  ...
  public void setWhenR(String tfqn, AlloyExpr exp) {
    ...
    this.tt.put(
        tfqn,
        new TransEntry(
            entry.params, entry.fromR, entry.onR, exp, entry.gotoR, entry.sendR, entry.doR));
  }

  public void setDoR(String tfqn, AlloyExpr exp) {
    TransEntry entry = this.tt.get(tfqn);
    this.tt.put(
        tfqn,
        new TransEntry(
            entry.params, entry.fromR, entry.onR, entry.whenR, entry.gotoR, entry.sendR, exp));
  }
