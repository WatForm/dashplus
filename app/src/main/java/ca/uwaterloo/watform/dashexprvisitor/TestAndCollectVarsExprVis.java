...
public class TestAndCollectVarsExprVis implements AlloyExprVis<Set<String>> {
  ...
  */
  ...
  public Set<String> visit(AlloyIteExpr iteExpr) {
    return mergeSets(
        this.visit(iteExpr.cond), mergeSets(this.visit(iteExpr.conseq), this.visit(iteExpr.alt)));
  }

  public Set<String> visit(AlloyLetExpr letExpr) {
    return mergeSets(
        flatten(mapBy(listToSet(letExpr.asns), a -> this.visit(a.expr))), this.visit(letExpr.body));
  }
