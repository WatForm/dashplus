...
public class InitializePA {
  ...
  public void createABVMap() {
    ...
    for (AlloyExpr e : preds) {
      ...
      if (eStr.contains(confName) || eStr.contains(transTakenName) || eStr.contains(eventsName)) {
        continue;
      }
    ...
    for (int i = 0; i < predList.size() - 1; i++) {
      for (int j = i + 1; j < predList.size(); j++) {
        ...
        if (!PredAbsUtil.checkSAT(Set.of(iff), queryModel, false, this.scope)) {
          ...
          untransDiscardedCAFMap.put(translatedPreds.get(e2), AlloyNot(translatedPreds.get(e1)));
          CAFs.remove(e2);
        }
      }
    }
        ...
        if (pname == null) { // in case of commands like "check/run {expr} for..."
          ...
          if (block != null) {
            ...
            throw AlloyCtorError.xorFields(
                this.cmdDecl.pos, "invoQname", "constrBlock", "AlloyCmdPara.CommandDecl");
          }
    ...
    for (int i = 0; i < ABVNameCAFTransMap.size() - 1; i++) {
      for (int j = i + 1; j < ABVNameCAFTransMap.size(); j++) {
        ...
        if (!PredAbsUtil.checkSAT(Set.of(i_imp_j), queryModel, false, this.scope)) {
          retList.add(
              AlloyImplies(
                  dsl.AlloyIsTrue(getVarDashRef(iname)), dsl.AlloyIsTrue(getVarDashRef(jname))));
        }
        if (!PredAbsUtil.checkSAT(Set.of(i_imp_not_j), queryModel, false, this.scope)) {
          retList.add(
              AlloyImplies(
                  dsl.AlloyIsTrue(getVarDashRef(iname)), dsl.AlloyIsFalse(getVarDashRef(jname))));
        }
        if (!PredAbsUtil.checkSAT(Set.of(j_imp_i), queryModel, false, this.scope)) {
          retList.add(
              AlloyImplies(
                  dsl.AlloyIsTrue(getVarDashRef(jname)), dsl.AlloyIsTrue(getVarDashRef(iname))));
        }
        if (!PredAbsUtil.checkSAT(Set.of(not_j_imp_i), queryModel, false, this.scope)) {
          retList.add(
              AlloyImplies(
                  dsl.AlloyIsFalse(getVarDashRef(jname)), dsl.AlloyIsTrue(getVarDashRef(iname))));
        }
      }
    }
  ...
  private void addCAFPreds() {
    for (String v : ABVNameCAFTransMap.keySet()) {
      ...
      queryModel.addPredPara(
          new AlloyPredPara(
              new AlloyQnameExpr(bPredPrefix + v), dsl.curDecls(), new AlloyBlock(List.of(caf))));
    }
  }
