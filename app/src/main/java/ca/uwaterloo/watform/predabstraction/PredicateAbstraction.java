...
import ca.uwaterloo.watform.dashexprvisitor.CollectDashRefVis;
...
public class PredicateAbstraction {
  ...
  // This map is used for CAF clustering and contains CAFs without translating the Expr
  ...
  public void createABVmap() {
    ...
    if (cmdBody != null) {
      // preds.addAll((new AlloyExprDecomposer()).decompose(cmdBody));
    ...
    // for now, forget this loop because it's supposed to add the CAFs without translating
    // but something is off. don't need this until clustering so added to todo
            ...
            for (List<AlloyExpr> combo : subCombos) {
              ...
              if (!uflag) {
                if (!PredAbsUtil.checkSAT(listToSet(combo), queryModel, false, scope)) {
                  unsatList.add(combo);
                  System.out.println("In addCAFDepInvs(): CAF Dependency Check, added UNSAT combo");
                }
              }
            }
          } else {
            ...
            if (e instanceof AlloyQuantificationExpr) {
              AlloyExpr absE =
                  ((AlloyQuantificationExpr) e)
                      .rebuild(createAbsExpr(((AlloyQuantificationExpr) e).body, true));
              absEList.add(absE);
            }
          }
